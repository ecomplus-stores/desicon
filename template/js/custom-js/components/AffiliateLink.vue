...
export default {
  ...
  methods: {
    ...
    share () {
      const text = 'Compre tudo que você precisa em construção, utilidades e ferramentas de qualidade ' +
        'com desconto através desse link!';
      const share = async () => {
        try {
          await navigator.share({
            title: 'Convite especial: Ganhe R$ x,00',
            text,
            url: this.link
          })
        } catch (err) {
