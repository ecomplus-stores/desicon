  ...
  export default {
    ...
    created () {
      ...
      if (this.product && this.product._id) {
        const cartItem = ecomCart.parseProduct(sanitizeProductBody(this.product))
        const productMetafieldQuantity = this.product.metafields.find(meta => meta.field === 'quantity')
        if (productMetafieldQuantity && productMetafieldQuantity.value) {
            cartItem.quantity = Number(productMetafieldQuantity.value)
            ...
            for (let i = 0; i < data.result.length; i++) {
                ...
                if (validated && !error && response && response.discount_rule && response.discount_rule.extra_discount) {
                    this.atacado.value = response.discount_rule.extra_discount.value
                    this.atacado.quantity = cartItem.quantity
                    this.atacado.rotulo = response.discount_rule.label
                }
            }
            })
        }
    ...
    mounted () {
      window.mainProductGallery_ = this.body.pictures ? [...this.body.pictures] : []
      window.mainProductGallery = this.body.pictures ? [...this.body.pictures] : []
